...
public interface ReleaseHistoryRepository extends PagingAndSortingRepository<ReleaseHistory, Long> {
  ...
  @Modifying
  @Query("update ReleaseHistory set IsDeleted = true, DeletedAt = round(date_part('epoch',now())), DataChange_LastModifiedBy = ?4 where AppId=?1 and ClusterName=?2 and NamespaceName = ?3 and IsDeleted = false")
