...
public interface NamespaceRepository extends PagingAndSortingRepository<Namespace, Long> {
  ...
  @Modifying
  @Query("update Namespace set IsDeleted = true, DeletedAt = round(date_part('epoch',now())), DataChange_LastModifiedBy = ?3 where AppId=?1 and ClusterName=?2 and IsDeleted = false")
