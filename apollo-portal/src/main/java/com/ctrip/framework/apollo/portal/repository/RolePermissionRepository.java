...
public interface RolePermissionRepository extends PagingAndSortingRepository<RolePermission, Long> {
  ...
  @Modifying
  @Query("UPDATE RolePermission SET IsDeleted = true, DeletedAt = round(date_part('epoch',now())), DataChange_LastModifiedBy = ?2 WHERE PermissionId in ?1 and IsDeleted = false")
